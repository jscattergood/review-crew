...
class BaseExporter(ABC):
    ...
    @abstractmethod
    def export(
        self, result: Any, output_path: Path, metadata: Optional[ExportMetadata] = None
    ) -> bool:
...
class JSONExporter(BaseExporter):
    ...
    def _convert_to_dict(
        self, result: Any, metadata: Optional[ExportMetadata]
    ) -> Dict[str, Any]:
        ...
        if hasattr(result, "context_results") and result.context_results:
            for context in result.context_results:
                context_data = {
                    "contextualizer_name": getattr(
                        context, "contextualizer_name", "Unknown"
                    ),
        ...
        export_data["summary"] = {
            ...
            "has_errors": hasattr(result, "review_errors")
            and bool(result.review_errors),
        }
...
<body>
    <div class="container">"""
        ...
        if metadata:
            ...
            html_parts.append(
                f"<strong>Generated:</strong> {metadata.export_timestamp}<br>"
            )
            html_parts.append(f"<strong>Format:</strong> {metadata.export_format}<br>")
            html_parts.append(
                f"<strong>Review Type:</strong> {metadata.review_type}<br>"
            )
            html_parts.append(
                f"<strong>Documents:</strong> {metadata.document_count}<br>"
            )
            html_parts.append(
                f"<strong>Reviewers:</strong> {metadata.reviewer_count}<br>"
            )
        ...
        if hasattr(result, "analysis_results") and result.analysis_results:
            ...
            for i, analysis in enumerate(result.analysis_results, 1):
                ...
                html_parts.append(
                    f"<div>{self._format_content_html(analysis_content)}</div>"
                )
                html_parts.append("</div>")
        ...
        if hasattr(result, "context_results") and result.context_results:
            ...
            for i, context in enumerate(result.context_results, 1):
                contextualizer_name = getattr(
                    context, "contextualizer_name", f"Contextualizer {i}"
                )
                ...
                html_parts.append(
                    f"<div>{self._format_content_html(context_content)}</div>"
                )
                html_parts.append("</div>")
...
class ExportManager:
    ...
    def _create_metadata(
        self, result: Any, format_type: str, metadata: Optional[Dict[str, Any]]
    ) -> ExportMetadata:
        ...
        return ExportMetadata(
            ...
            review_type=(
                metadata.get("review_type", "multi-document")
                if metadata
                else "multi-document"
            ),
