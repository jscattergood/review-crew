...
class DocumentValidator:
    ...
    def validate_document(self, file_path: Path) -> Tuple[List[ValidationResult], DocumentMetadata]:
    ...
    def validate_document_collection(
        ...
        if expected_docs:
            missing_results = self._check_missing_documents(directory_path, expected_docs)
            if missing_results:
                results["_collection_issues"] = (missing_results, DocumentMetadata(0, 0, 0, 0))
    ...
    def _validate_content(self, content: str, metadata: DocumentMetadata) -> List[ValidationResult]:
    ...
    def _validate_quality(self, content: str, metadata: DocumentMetadata) -> List[ValidationResult]:
    ...
    def _get_expected_documents(self, manifest_config: Optional[Dict[str, Any]]) -> List[str]:
    ...
    def _check_missing_documents(self, directory_path: Path, expected_docs: List[str]) -> List[ValidationResult]:
        """Check for missing expected documents."""
        results = []
    ...
    def generate_validation_report(
        self, validation_results: Dict[str, Tuple[List[ValidationResult], DocumentMetadata]]
    ) -> str:
                ...
                if info:
                    ...
                    for info_item in info:
                        ...
                        if info_item.suggestion:
                            report_lines.append(f"  *Suggestion: {info_item.suggestion}*")
                    report_lines.append("")
            else:
