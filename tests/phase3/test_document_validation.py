...
from src.validation.document_validator import (
    DocumentValidator, ValidationLevel, ValidationResult, DocumentMetadata
)
...
Final paragraph here."""
    ...
    def test_validate_content_word_count(self):
        ...
        metadata = DocumentMetadata(word_count=2, character_count=9, paragraph_count=1, sentence_count=1)
        ...
        metadata = DocumentMetadata(word_count=60, character_count=len(long_content), paragraph_count=1, sentence_count=1)
    ...
    def test_validate_content_empty(self):
        ...
        validator = DocumentValidator()
        metadata = DocumentMetadata(word_count=0, character_count=0, paragraph_count=0, sentence_count=0)
...
"""
    ...
    def test_validate_collection_with_manifest(self, tmp_path):
        ...
        if "_collection_issues" in results:
            ...
            missing_errors = [r for r in collection_results if "missing" in r.message.lower()]
    ...
    def test_generate_validation_report(self):
        ...
        validation_results = {
            "test.md": (
                [
                    ValidationResult(ValidationLevel.WARNING, "Test warning", suggestion="Fix this"),
                    ValidationResult(ValidationLevel.INFO, "Test info")
                ],
                DocumentMetadata(word_count=100, character_count=500, paragraph_count=3, sentence_count=5, reading_level=10.5)
            ),
            "good.md": (
                [],  # No issues
                DocumentMetadata(word_count=200, character_count=1000, paragraph_count=4, sentence_count=8)
            )
        }
    ...
    def test_syllable_counting(self):
        ...
        for word, expected_min in test_cases:
            ...
            assert syllables >= expected_min, f"Word '{word}' should have at least {expected_min} syllables"
