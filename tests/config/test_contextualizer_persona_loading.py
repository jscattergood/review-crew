...
""")
    ...
    def test_load_contextualizer_personas_by_category(self, temp_personas_dir):
        ...
        business_contextualizers = loader.load_contextualizer_personas_by_category(
            ["business"]
        )
        ...
        academic_contextualizers = loader.load_contextualizer_personas_by_category(
            ["academic"]
        )
        ...
        multiple_contextualizers = loader.load_contextualizer_personas_by_category(
            ["business", "academic"]
        )
        assert len(multiple_contextualizers) == 2

    def test_load_contextualizer_personas_by_category_not_found(
        self, temp_personas_dir
    ):
        """Test loading contextualizers from non-existent category."""
        loader = PersonaLoader(personas_dir=temp_personas_dir)

        # Test with non-existent category
        contextualizers = loader.load_contextualizer_personas_by_category(
            ["nonexistent"]
        )
        assert len(contextualizers) == 0

    def test_load_contextualizers_from_manifest_categories_only(
        self, temp_personas_dir
    ):
        """Test loading contextualizers from manifest with categories only."""
        loader = PersonaLoader(personas_dir=temp_personas_dir)

        manifest_config = {"contextualizer_categories": ["business", "academic"]}
    ...
    def test_load_contextualizers_from_manifest_both_categories_and_names(
        self, temp_personas_dir
    ):
    ...
    def test_load_contextualizers_from_manifest_deduplication(self, temp_personas_dir):
        ...
        manifest_config = {
            "contextualizer_categories": [
                "business"
            ],  # This loads "Business Context Analyst"
            "contextualizers": [
                "Business Context Analyst"
            ],  # This also tries to load the same one
        }
    ...
    def test_load_contextualizers_from_manifest_no_contextualizer_config(
        self, temp_personas_dir
    ):
        ...
        manifest_config = {
            "reviewer_categories": [
                "academic"
            ],  # Other config, but no contextualizer config
