...
""",
    ...
    def test_load_reviewer_personas_no_directory(self, tmp_path):
        ...
        loader = PersonaLoader(
            project_root=tmp_path, personas_dir=tmp_path / "nonexistent"
        )
    ...
    @patch("pathlib.Path.exists", return_value=True)
    def test_load_reviewer_personas_success(
        self, mock_exists, mock_load_personas, mock_persona
    ):
    ...
    @patch("src.config.persona_loader.PersonaLoader._load_personas_from_dir")
    @patch("pathlib.Path.exists", return_value=True)
    def test_load_analyzer_personas(
        self, mock_exists, mock_load_personas, mock_persona
    ):
    ...
    @patch("src.config.persona_loader.PersonaLoader._load_personas_from_dir")
    @patch("pathlib.Path.exists", return_value=True)
    def test_load_contextualizer_personas(
        self, mock_exists, mock_load_personas, mock_persona
    ):
    ...
    def test_load_all_persona_types(
        self, mock_contextualizers, mock_analyzers, mock_reviewers, mock_persona
    ):
