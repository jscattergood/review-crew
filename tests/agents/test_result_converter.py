...
class TestResultConverter:
    ...
    def simple_graph_result(self):
        ...
        review_multi_result = MultiAgentResult(
            results={"test_reviewer": NodeResult(result=review_agent_result, status=Status.COMPLETED)},
            execution_time=0.2,
            execution_count=1
        )
        
        review_node_result = NodeResult(result=review_multi_result, status=Status.COMPLETED)
    ...
    def test_convenience_function(self, simple_graph_result):
        ...
        result = convert_graph_result_to_conversation(simple_graph_result, "original content")
