...
"""
    ...
    async def test_mock_review_workflow(self, sample_python_code):
        ...
        with (
            ...
            mock_strands_agent.invoke_async = AsyncMock(
                return_value="MOCK REVIEW: This is a mock review response with security and technical feedback."
            )
            mock_strands_agent.__call__ = Mock(
                return_value="MOCK REVIEW: This is a mock review response with security and technical feedback."
            )
            ...
            mock_review_agent.invoke_async_legacy = AsyncMock(
                return_value="MOCK REVIEW: This is a mock review response with security and technical feedback."
            )
            ...
            try:
                ...
                )  # Disable analysis for simpler test
    ...
    async def test_context_integration(self, sample_content, sample_context):
        ...
        with (
            ...
            mock_strands_agent = Mock()
            mock_strands_agent.invoke_async = AsyncMock(
                return_value="Review with context completed"
            )
            mock_strands_agent.__call__ = Mock(
                return_value="Review with context completed"
            )
            ...
            try:
                ...
                result = await manager.run_review(
                    sample_content, context_data=sample_context
                )
...
"""
        ...
        with (
            ...
            mock_strands_agent = Mock()
            mock_strands_agent.invoke_async = AsyncMock(
                return_value="MOCK FILE REVIEW: This API documentation has security issues."
            )
            mock_strands_agent.__call__ = Mock(
                return_value="MOCK FILE REVIEW: This API documentation has security issues."
            )
            ...
            mock_review_agent.review.return_value = (
                "MOCK FILE REVIEW: This API documentation has security issues."
            )
            mock_review_agent.get_info.return_value = {
    ...
    async def test_expected_feedback_patterns(self, sample_python_code):
        ...
        with (
            ...
            mock_strands_agent = Mock()
            mock_strands_agent.invoke_async = AsyncMock(
                return_value="SECURITY REVIEW: Critical security vulnerabilities found including MD5 password hashing and plaintext password logging. TECHNICAL REVIEW: Code quality issues detected - missing error handling and input validation best practices."
            )
            mock_strands_agent.__call__ = Mock(
                return_value="SECURITY REVIEW: Critical security vulnerabilities found including MD5 password hashing and plaintext password logging. TECHNICAL REVIEW: Code quality issues detected - missing error handling and input validation best practices."
            )
            ...
            try:
                ...
                all_feedback = " ".join(
                    [r.feedback.lower() for r in result.reviews if not r.error]
                )
                ...
                assert len(found_patterns) > 0, (
                    f"No expected patterns found in feedback: {all_feedback}"
                )
...
class TestSystemHealth:
    ...
    def test_configuration_paths(self):
        ...
        if not personas_dir_exists:
            pytest.skip(
                "Personas directory not configured - this is expected in test environment"
            )
    ...
    def test_test_inputs_available(self, test_files_dir):
        ...
        test_files = (
            list(test_files_dir.glob("*.py"))
            + list(test_files_dir.glob("*.md"))
            + list(test_files_dir.glob("*.html"))
        )
