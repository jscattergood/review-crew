...
class TestMultiDocumentAnalyzerPersonas:
    ...
    def test_application_coherence_analyzer_loads(self):
        ...
        persona_path = self.personas_dir / "analyzers" / "application_coherence_analyzer.yaml"
        assert persona_path.exists(), "Application Coherence Analyzer persona file should exist"
    ...
    def test_cross_document_themes_analyzer_loads(self):
        ...
        persona_path = self.personas_dir / "analyzers" / "cross_document_themes_analyzer.yaml"
        assert persona_path.exists(), "Cross-Document Themes Analyzer persona file should exist"
    ...
    def test_competitive_analysis_specialist_loads(self):
        ...
        persona_path = self.personas_dir / "analyzers" / "competitive_analysis_specialist.yaml"
        assert persona_path.exists(), "Competitive Analysis Specialist persona file should exist"
    ...
    def test_all_multi_document_analyzers_have_required_fields(self):
        ...
        for persona_path in analyzer_paths:
            ...
            assert "{content}" in persona.prompt_template, f"Analyzer should have content placeholder: {persona_path}"
    ...
    def test_analyzer_prompt_templates_include_synthesis_focus(self):
        ...
        synthesis_keywords = [
            "synthesis", "synthesize", "holistic", "meta", "cross-document", 
        ...
        for persona_path in analyzer_paths:
            ...
            found_keywords = [keyword for keyword in synthesis_keywords if keyword in prompt_lower]
    ...
    def test_analyzers_differentiate_from_reviewers(self):
        ...
        analyzer_path = self.personas_dir / "analyzers" / "application_coherence_analyzer.yaml"
        reviewer_path = self.personas_dir / "reviewers" / "academic" / "application_reviewer.yaml"
        ...
        assert "review feedback" in analyzer.prompt_template.lower() or "reviewer" in analyzer.prompt_template.lower()
        
        # Reviewer should focus on evaluating content
        assert "{reviews}" not in reviewer.prompt_template
        assert "evaluation" in reviewer.prompt_template.lower() or "assess" in reviewer.prompt_template.lower()
        
        # Analyzer should mention synthesis/meta-analysis
        analyzer_lower = analyzer.prompt_template.lower()
        assert any(word in analyzer_lower for word in ["synthesis", "synthesize", "meta", "holistic"])
    ...
    def test_analyzer_directory_structure(self):
        ...
        for filename in new_analyzer_files:
            ...
            assert analyzer_path.exists(), f"Analyzer should exist in analyzers directory: {filename}"
